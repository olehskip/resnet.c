...
void Conv2d::forward(FloatTensor &x, FloatTensor &out)
{
    ...
    conv2dForwardKernel<<<blocks, block_size>>>(x.data, out.data, weight.data, kernel_size, stride,
                                                padding, h_out, w_out, B, in_channels, out_channels,
                                                H, W);
...
void BatchNorm2d::forward(FloatTensor &x, FloatTensor &out)
{
    ...
    batchNorm2dForwardKernel<<<bn_blocks, bn_block_size>>>(x.data, out.data, weight.data, bias.data,
                                                           mean.data, var.data, B, channels_num,
                                                           conv_w_out * conv_h_out);
    cudaDeviceSynchronize();
    gpuAssert(cudaGetLastError(), __FILE__, __LINE__);
}
...
void Pool2d::avgforward(FloatTensor &x, FloatTensor &out)
{
    ...
    avgPool2dKernel<<<blocks, block_size>>>(x.data, out.data, kernel_size, stride, padding, out_h,
                                            out_w, B, C, H, W);
    cudaDeviceSynchronize();
    gpuAssert(cudaGetLastError(), __FILE__, __LINE__);
}
...
void Pool2d::maxforward(FloatTensor &x, FloatTensor &out)
{
    ...
    const auto block_size = dim3(1, 1, 1);
    const auto blocks = dim3(H * W, B, C);
    maxPool2dKernel<<<blocks, block_size>>>(x.data, out.data, kernel_size, stride, padding, out_h,
                                            out_w, B, C, H, W);
    cudaDeviceSynchronize();
    gpuAssert(cudaGetLastError(), __FILE__, __LINE__);
}
