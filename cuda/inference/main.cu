...
void layerForward(Layer &layer, FloatTensor &x)
{
    ...
    for (auto &block : layer.blocks) {
        ...
        block.bn2.forward(block.act2_out, block.act2_out);
        ...
        addForward(block.act3_out, block.downsample ? block.downsample->act : *y, block.act3_out);
...
int main()
{
    const uint64_t B = 1, /* C = 3, */ H = 224, W = 224;
    std::cout << "Started\n";

    ResnetModel resnet_model = createResnet152();
    std::cout << "created model\n";

    FloatTensor inp = FloatTensor::loadToCuda("test_bins/ILSVRC2012_val_00004749.bin")
                          .view(Shape({B, resnet_model.conv1.in_channels, W, H}));
    resnet152Forward(resnet_model, inp);

    auto cpu_out = resnet_model.fc_out.cpu();
    const uint64_t classes = 1000;
    assert(classes == cpu_out.shape().back());
    for (uint64_t b = 0; b < B; ++b) {
        uint64_t mx_idx = 0;
        for (uint64_t i = 1; i < classes; ++i) {
            if (cpu_out.data()[b * classes + mx_idx] < cpu_out.data()[b * classes + i]) {
                mx_idx = i;
            }
        }
        std::cout << "max index is " << mx_idx << std::endl;
    }
